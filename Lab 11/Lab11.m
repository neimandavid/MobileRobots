...
%start = [0.3048; 0.3048; pi/2];
start = [0.6096; 0.6096; pi/2];
...
% while true
%    d.drive(robot, 2)
%    [lidarpose, ranges] = getLidarPose(pose);
%    pose = fusePoses(pose, lidarpose);
%    %pose = fusePoses(pose, lidarpose);
...
function fusedpose = fusePoses(currentPose, lidarPose)
    ...
    fusedpose(3) = currentPose(3) + gain*thetadiff;
    while(fusedpose(3) < -pi)
        fusedpose(3) = fusedpose(3) + 2*pi;
    end
    while fusedpose(3) > pi
        fusedpose(3) = fusedpose(3) - 2*pi;
    end
...
function laserListener(handle, event)
    global encpose; global timeout; global lpose; global ltimestamp;
    [lpose, ltimestamp] = getLidarPose(encpose, timeout);
end
...
function [pose, timestamp, rangesout] = getLidarPose(pose, timeout, oldtime)
    ...
    dpose = 10^-2;
    cutoff = dpose;
    [ranges, timestamp] = getNiceRanges(-5);
...
function cdata = findSail2(ranges, sailwidth)
    ...
    while x < 2*numpts
       ...
       for y = 1:numpts
          ...
          if norm(newpt(3:4)-oldpt(3:4)) > 0.05
              break
              %Found a 5 cm gap
          end
       ...
       if norm(startpt(3:4)-oldpt(3:4)) < .5*sailwidth
          if debug
              'Too small'
          end
          x = x+y;
       ...
       if norm(startpt(3:4)-oldpt(3:4)) > 1.5*sailwidth
          if debug
              'Too big'
          end
          x = x+y;
...
function cdata = findSail(ranges, sailwidth)
    ...
    for x = 1:numpts
       ...
       for y = 1:numpts
           ...
           if norm(startpt(3:4) - newpt(3:4)) > sailwidth
...
%            end
       ...
       if lambda(1) < 1.3 && ~secondgapflag
           if debugSail
                'Can add more points and stay linear'
           end
           continue %Skip stuff with really high variance
       end
       if debugSail
...
function [niceRanges, timestamp] = getNiceRanges(fudge)
    ...
    %r, theta, x, y
    [ranges, timestamp] = getRanges(fudge);
...
function [ranges, timestamp] = getRanges(fudge)
    global minrange, global maxrange;
    [ranges, timestamp] = rawRanges(fudge);
    for x = 1:360
...
function [ranges, timestamp] = rawRanges(fudge)
    if nargin == 0
        fudge = 0;
    end
    if fudge < 0
    ...
    if(~isSim)
        ...
        timestamp = double(robot.laser.LatestMessage.Header.Stamp.Sec) + double(robot.laser.LatestMessage.Header.Stamp.Nsec)/1e9;
        ranges = zeros(1, 360);
...
function [poseout, goalposeout] = moveRelPos(xtarget, ytarget, thtarget, vMax, currentPose, curgoalpose)
    ...
    while true  
       ...
       if (norm(newenc(1:2)-oldenc(1:2)) > 1 || newenc(3) < oldenc(3)) && consecbadenccount < 3 && loopcount > 5
           'Bad encoder data?'
           newenc-oldenc
           loopcount
           badenccount = badenccount + 1
       ...
       if lidartime > oldlidartime || true
          %Fuse
          lidarpose = lpose;
          temp(1:3) = fusePoses(temp(1:3), lidarpose);
          x = temp(1); y = temp(2); th = temp(3);
          oldlidartime = ltimestamp;
       end
       if lidartime < oldlidartime
           'Lidar can time-travel too!'
       end
