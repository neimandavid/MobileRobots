...
% plot(rangeData(3, :), rangeData(4, :), 'o');
% findSail(rangeData, 12.7);

for x = 1:3
    %Comment if we don't want the intermediate step
    goToSail(0.127, -0.23, 0.4, pose, goalpose);
    'First move'
    goToSail(0.127,-0.13, 0.2, pose, goalpose);
    %pause(1);
    %move(0.07, 0.07);
    %pause(1);
    %move(0, 0);
    ...
    moveRelPos(.07, 0, 0, 0.05, pose, pose);
    'Second move'
    forksUp();
    pause(1);
    forksDown();
    ...
    backThenTurn(0.07, pi, 0.2, pose, goalpose);
    pause(10);
end
